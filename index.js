...
// ================================================================
// ADMIN — delete a user account completely
// ----------------------------------------------------------------
// A plain Firestore write from the browser can never delete someone
// else's Firebase Auth login — only the Admin SDK can do that, which
// is why this needs a Cloud Function. Deletes the Auth account, the
// main user document, and their owned records across collections.
// ================================================================
const USER_OWNED_COLLECTIONS = [
  'transactions', 'loans', 'investments', 'kycRequests', 'disputes',
  'withdrawals', 'fundingRequests', 'fundings', 'pinRequests',
  'pwResetRequests', 'airtimeCashRequests', 'easyBuy', 'easyBuyCashPayments',
  'virtualCards', 'investmentRequests', 'loginActivity', 'rewards',
  'beneficiaries', 'scheduledPayments', 'billReminders', 'splitRequests',
  'savings', 'savingsTransactions', 'notifications', 'financeNotifications',
  'promoUses',
];

exports.adminDeleteUser = functions.https.onCall(async (data, context) => {
  if (!context.auth || (context.auth.token.email || '').toLowerCase() !== SUPER_ADMIN_EMAIL) {
    throw new functions.https.HttpsError('permission-denied', 'Only the Super Admin can delete user accounts.');
  }
  const { uid } = data || {};
  if (!uid) {
  ...
  if (uid === context.auth.uid) {
    throw new functions.https.HttpsError('invalid-argument', "You can't delete your own account.");
  }

  try {
    // 1) Delete the Firebase Auth login itself.
    await admin.auth().deleteUser(uid).catch((e) => {
      // If the Auth user is already gone, keep going and clean up Firestore anyway.
      if (e.code !== 'auth/user-not-found') throw e;
    });

    // 2) Delete their owned documents in every collection that keys by userId.
    for (const coll of USER_OWNED_COLLECTIONS) {
    ...
    // 3) Delete their support ticket thread (doc ID = uid) and its messages.
    const ticketRef = db.collection('supportTickets').doc(uid);
    const msgsSnap = await ticketRef.collection('messages').get();
    const msgBatch = db.batch();
    msgsSnap.docs.forEach((d) => msgBatch.delete(d.ref));
    if (!msgsSnap.empty) await msgBatch.commit();
    await ticketRef.delete().catch(() => {});

    // 4) Delete the main user profile document last.
    await db.collection('users').doc(uid).delete();

    return { success: true };
  } catch (e) {
    console.error('adminDeleteUser error:', e);
    throw new functions.https.HttpsError('internal', 'Could not fully delete this user: ' + e.message);
  }
});
